...
fn prepare() -> (Vec<Complex<f32>>, CepstrumExtractor<f32>) {
    (
        vec![0f32; BUF_LEN].to_complex_vec(),
...
fn rceps(b: divan::Bencher) {
    ...
    b.bench_local(|| {
        divan::black_box(extractor.rceps_to_vec(&buf));
    });
}
...
fn cceps(b: divan::Bencher) {
    let (buf, extractor) = prepare();
    b.bench_local(|| {
        divan::black_box(extractor.cceps_to_vec(&buf));
    });
}